...
def run_fabric_patterns():
    try:
        with open("temp_summary.txt", "w") as file:
            header = "CHATGPT SUMMARY CREATED BY https://github.com/danielmiessler/fabric ----- USING **SUMMARIZE** FABRIC PATTERN\n\n"
            file.write(header)
        ...
        with open("temp_summary.txt", "a") as file:
            file.write(summarize_result.stdout)
        ...
        with open("temp_summary.txt", "a") as file:
            file.write(report_result.stdout)
        ...
        with open("temp_summary.txt", "a") as file:
            file.write(references_result.stdout)
...
# Function to read summary from temp file
def read_summary_from_temp_file():
    with open("temp_summary.txt", "r") as file:
        summary = file.read()
    return summary
...
def process_pdfs(pdf_folder_path):
    for filename in os.listdir(pdf_folder_path):
        if filename.endswith(".pdf"):
            ...
            run_fabric_patterns()
            summary_text = read_summary_from_temp_file()
